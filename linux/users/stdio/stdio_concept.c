...
---->流与文件描述符
文件描述符:
如果我们用fd表示，该fd可以是open、pipe、dup、dup2和creat等调用返回的结果，在linux系统中，设备也是以文件的形式存在，
要对该设备进行操作就必须先打开这个文件，打开这个文件就会，就会获得这个文件描述符，它是个很小的正整数，它是一个索引值，指向内核
为每一个进程所维护的该进程打开文件的记录表。文件描述符的优点：兼容POSIX标准，许多Linux和 UNIX系统调用都依赖于它。文件描述符的缺
点：不能移植到UNIX以外的系统上去，也不直观。fd只是一个索引.

文件指针:
C语言中使用的是文件指针而不是文件描述符做为I/O的句柄."文件指针(file pointer)"指向进程用户区中的一个被称为FILE结构的数据结构。
FILE结构包括一个缓冲区和一个文件描述符值.而文件描述符值是文件描述符表中的一个索引.从某种意义上说文件指针就是句柄的句柄。
...
2 行缓冲 Line buffered
写了一行之后才进行实际的I/O操作。缓冲行的长度是固定的，写满了(此时可能还没到一行的结束)也进行I/O操作。
3 不带缓冲 Unbuffered
标准I/O库不对字符进行行缓冲存储，立即写至相关联的打开文件上。
标准出错流stderr通常是不带缓冲的。
---->文件使用方式
