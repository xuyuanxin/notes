...
A year value will be handled as described under Year 2000 (Y2K) issues above. A -1 a-
rgument as the daylight savings flag, passed to mktime() will usually result in the -
...
, or having elements of the wrong type, a TypeError is raised.
'''
class time.struct_time

''' time.localtime([secs])
Like gmtime() but converts to local time. If @secs is not provided or None, the curr-
ent time as returned by time() is used. The dst flag is set to 1 when DST applies  to 
the given time.
'''
def localtime([secs]):
    pass
...
n though the time is always returned as a floating point number, not all systems pro-
vide time with a better precision than 1 second. While this function normally returns 
non-decreasing values, it can return a lower value than a previous call if the system 
clock has been set back between the two calls.
'''
def time():
   pass
