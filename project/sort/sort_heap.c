...
二叉堆一般用数组来表示。
如果存储数组的下标基于0，共有n个元素。[0,n-1]
1 标为i的节点的子节点是2i + 1与 2i + 2；其父节点的下标是(i - 1)/2(取下整)。
2 [n/2,n-1]都是叶子节点
...
/*-----------------------------------------------------------------------------------
 ...
 @max  : 
    max小于等于数组的个数
 功能  : 
    array[i]的左右子树都满足最大堆性质,调用此函数让以array[i]为根的树也满足最大堆性质。
-----------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------
 ...
 功能 : 
    建立最大堆
 实现 : 
    [n/2,n-1]都是叶子节点，所有从n/2-1开始建堆。注意必须从n/2-1开始建堆，建堆的过程就
    是把"大元素"提拔到树根的过程。
-----------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------
 ...
 功能 : 
    堆排序(从小到大)
 方法 : 
    1 首先在整个数组上建立最大堆，此时最大的元素是array[0]。
